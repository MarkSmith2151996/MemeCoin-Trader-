...
class DirectExecutor(ExecutionAdapter):
    ...
    async def buy(self, mint_address: str, amount_sol: float) -> TradeResult:
        ...
        signature, status, slot, used_jito = await self._submit_and_confirm(instructions)
    ...
    async def sell(self, mint_address: str, token_amount: float) -> TradeResult:
        ...
        signature, status, slot, used_jito = await self._submit_and_confirm([instruction])
        fill = await self._get_transaction_fill(signature, mint_address)
    ...
    async def _submit_and_confirm(self, instructions) -> tuple[str, str, int | None, bool]:
        ...
        return signature, status, slot, used_jito
