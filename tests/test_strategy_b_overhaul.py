...
def test_holder_gate_accepts_concentrated_fresh_tokens() -> None:
    assert MAX_TOP10_HOLDER_PCT == 100.0
    for age in (0.5, 3, 8, 20):
        warn_pct, hard_pct = _age_holder_tier(age)
        assert hard_pct == 100.0
        assert 100.0 <= hard_pct  # exact-100% reports pass the inclusive comparison
